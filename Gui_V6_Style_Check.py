...
def process_return():
    ...
    for remaining in active_hires:
        print_to_txt(remaining["name"], remaining["item"],
                remaining["quantity"], remaining["receipt"])
...
def submit_item():
    ...
    if items == "":
        messagebox.showerror("Input Error",
                             "Number of items cannot be blank")
        return
    try:
        items = int(items)
    except ValueError:
        messagebox.showerror("Input Error",
                             "please enter an number without a decimal")
        return

    if items < 1:
        messagebox.showerror("Input Error",
                             "please enter an number that is more than (or equal to) 1")
        return
    elif items > 500:
        messagebox.showerror("Input Error",
                             "please enter an number that is less than 500.")
        return
    ...
    try:
        ...
        messagebox.showerror("Input Error",
                             "Receipt number must be a number")
        return

    # Appending the data to a list to keep track of the hired items
    active_hires.append({
        "name":name, "item":item_hired,
        "quantity":items, "receipt":receipt_num
    })
...
lbl_age = tk.Label(root, text="Name:", borderwidth=2,
...
lbl_receipt_num = tk.Label(root, text="Receipt Number:",
                           borderwidth=2, relief="solid", fg="black")
...
selected_item = ttk.Combobox(root, values=["Football", "Basketball", "Roadcone", "Pineapple pen"] ,
                             state="readonly")
selected_item.grid(row=2, column=1, sticky="e")
selected_item.current(0)

# Number of the item hired
lbl_number = tk.Label(root, text="Number hired:", borderwidth=2, 
                      relief="solid", fg="black")
...
lbl_return=tk.Label(root, text="Returns", borderwidth=2,
                    relief="solid", fg="black")
